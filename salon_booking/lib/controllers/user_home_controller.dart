...
class UserHomeController extends GetxController {
  ...
  Future<void> _initializeApp() async {
    try {
      ...
      await Future.wait([_loadUserCity(), fetchAvailableCities()]);
  ...
  Future<void> fetchNearbySalons({double? radius}) async {
    try {
      ...
      print(
        '📍 HOME: Fetching salons within ${searchRadius}km of ($lat, $lon)',
      );
  ...
  Future<void> fetchAvailableCities() async {
    try {
      ...
      final response = await http
          .get(Uri.parse('${ApiConfig.baseUrl}/salons/cities/'))
  ...
  Future<void> fetchSalonServices(String salonId) async {
    try {
      ...
      final response = await http
          .get(Uri.parse('${ApiConfig.baseUrl}/services/?salon=$salonId'))
          .timeout(const Duration(seconds: 10));
  ...
  Future<void> refreshAll() async {
    await Future.wait([fetchSalons(), fetchAvailableCities()]);
  }
